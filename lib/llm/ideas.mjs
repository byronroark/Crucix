...
function parseIdeasResponse(text) {
  const arr = extractJsonArray(text, {
    arrayKeys: ['ideas', 'items', 'trades', 'recommendations', 'data'],
    debugLabel: 'ideas',
  });
  if (!arr) return null;
...
function normalizeIdea(idea) {
  ...
  const title = idea.title || idea.name || idea.headline || '';
  let type = String(idea.type || idea.action || idea.position || '').toUpperCase();
  if (!type && title) type = 'WATCH';
  const confidence = String(idea.confidence || '').toUpperCase();
