...
Output ONLY a valid JSON object with an "ideas" array. No markdown fences, no prose outside JSON. Each array element:
{
  ...
  try {
    ...
    const preview = (result.text || '').replace(/\s+/g, ' ').substring(0, 400);
    ...
    console.warn('[LLM Ideas] No valid ideas parsed from response'
      + ` (finish=${finish}, outTokens=${outTok})`
...
function parseIdeasResponse(text) {
  const arr = extractJsonArray(text, {
    arrayKeys: ['ideas', 'trade_ideas', 'leverageable_ideas', 'items', 'trades', 'recommendations', 'data'],
    debugLabel: 'ideas',
  });
  if (!arr) return null;
  ...
  if (!normalized.length && arr.length) {
    console.warn(`[LLM Ideas] Parsed ${arr.length} raw items but none passed validation — keys in first: ${Object.keys(arr[0] || {}).join(', ')}`);
  }
...
function normalizeIdea(raw) {
  ...
  let idea = raw;
  if (idea.idea && typeof idea.idea === 'object') idea = { ...idea, ...idea.idea };
  if (idea.trade && typeof idea.trade === 'object') idea = { ...idea, ...idea.trade };

  const title = idea.title || idea.name || idea.headline || idea.thesis || '';
  let type = String(
    idea.type || idea.action || idea.position || idea.trade_type || idea.direction || idea.side || ''
  ).toUpperCase();
  if (!type && title) type = 'WATCH';
  const confidence = String(idea.confidence || idea.conviction || '').toUpperCase();
  if (!title) return null;
  const validTypes = new Set(['LONG', 'SHORT', 'HEDGE', 'WATCH', 'AVOID']);
  const validConf = new Set(['HIGH', 'MEDIUM', 'LOW']);
  return {
    title: String(title).substring(0, 120),
    type: validTypes.has(type) ? type : 'WATCH',
    ticker: idea.ticker || idea.symbol || idea.instrument || '',
    confidence: validConf.has(confidence) ? confidence : 'MEDIUM',
    rationale: idea.rationale || idea.text || idea.summary || idea.description || '',
    risk: idea.risk || idea.risks || '',
    horizon: idea.horizon || idea.timeframe || '',
    signals: Array.isArray(idea.signals) ? idea.signals.map(String) : [],
    source: 'llm',
  };
}
