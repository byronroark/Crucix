...
const VALID_TYPES = new Set(['rss', 'firecrawl', 'http-json']);
...
/** Normalize + validate a custom source payload for the user store. */
export function validateSource(raw, { partial = false } = {}) {
  const errors = [];
  if (!raw || typeof raw !== 'object') return { ok: false, errors: ['invalid payload'] };

  const type = raw.type || 'rss';
  ...
  if (!partial || raw.type !== undefined) {
    if (!VALID_TYPES.has(type)) errors.push('type must be rss, firecrawl, or http-json');
  }
  ...
  const refreshMinutes = raw.refreshMinutes != null
    ...
    : (type === 'firecrawl' ? 120 : type === 'http-json' ? 15 : 30);
  ...
  if (type === 'firecrawl') {
    firecrawl = {
      formats: Array.isArray(raw.firecrawl?.formats) ? raw.firecrawl.formats : ['markdown'],
      onlyMainContent: raw.firecrawl?.onlyMainContent !== false,
    };
  }
  ...
  if (type === 'http-json') {
    json = {
      itemsPath: String(raw.json?.itemsPath || raw.itemsPath || '').trim(),
      titleField: String(raw.json?.titleField || raw.titleField || 'title').trim() || 'title',
      urlField: String(raw.json?.urlField || raw.urlField || 'url').trim() || 'url',
      dateField: String(raw.json?.dateField || raw.dateField || 'date').trim() || 'date',
      contentField: String(raw.json?.contentField || raw.contentField || '').trim() || undefined,
      latField: String(raw.json?.latField || raw.latField || '').trim() || undefined,
      lonField: String(raw.json?.lonField || raw.lonField || '').trim() || undefined,
    };
  }
...
/** @deprecated use validateSource */
export function validateRssSource(raw, opts) {
  return validateSource({ ...raw, type: raw?.type || 'rss' }, opts);
}
...
export function addSource(payload) {
  const v = validateSource(payload);
