...
const DEFAULT_COUNT_THRESHOLDS = {
  ...
  thermal_hc: 25,          // ±25 high-confidence thermal hotspots
  ...
  sdr_total: 10,           // ±10 receivers in network total
  earthquake_total: 1,     // any new quake in feed matters
...
const COUNT_METRICS = [
  ...
  { key: 'thermal_hc', extract: d => d.thermal?.reduce((s, t) => s + (t.hc || 0), 0) || 0, label: 'Thermal High-Confidence' },
  ...
  { key: 'sdr_online', extract: d => d.sdr?.online || 0, label: 'SDR Receivers Online' },
  { key: 'sdr_total', extract: d => d.sdr?.total || 0, label: 'SDR Network Total' },
  { key: 'earthquake_total', extract: d => d.earthquakes?.total || 0, label: 'Earthquakes' },
...
function customItemKey(item) {
  ...
  const title = String(item.title || '').trim().substring(0, 200);
  const name = String(item.name || '').trim();
  return createHash('sha256').update(`${name}|${title}`).digest('hex').substring(0, 16);
}
...
export function computeDelta(current, previous, thresholdOverrides = {}, priorRuns = []) {
  ...
  // ─── New custom analyzed OSINT items (tag-aware) ─────────────────────

  if (config.customSignals?.enabled !== false) {
    const priorityTags = (config.customSignals?.priorityTags || [])
      .map(t => String(t).toLowerCase());
    const minMatches = config.customSignals?.minTagMatches ?? 1;
    const sources = priorRuns.length > 0 ? priorRuns : [previous];
    const prevCustomKeys = new Set(
      sources.flatMap(run => (run?.customAnalyzed || []).map(customItemKey)).filter(Boolean)
    );

    for (const item of (current.customAnalyzed || [])) {
      const key = customItemKey(item);
      if (!key || prevCustomKeys.has(key)) continue;
      const tags = (item.tags || []).map(t => String(t).toLowerCase());
      const titleLower = String(item.title || '').toLowerCase();
      const tagHits = priorityTags.filter(t => tags.includes(t) || titleLower.includes(t));
      const isPriority = tagHits.length >= minMatches;
      signals.new.push({
        key: `custom_analyzed:${key}`,
        label: item.name || 'Custom OSINT',
        text: item.title,
        item,
        reason: isPriority
          ? `New analyzed custom item [${tagHits.join(', ')}]: ${item.title}`
          : `New analyzed custom item: ${item.title}`,
        severity: isPriority ? 'high' : 'moderate',
        tags: tagHits,
      });
      if (isPriority) criticalChanges++;
    }
  }
